...
import structures.units.unitFireSpitter;
import structures.units.unitPurebladeEnforcer;
import utils.BasicObjectBuilders;
import utils.StaticConfFiles;

public class cardFireSpitter extends Card {
    public void UseCard(ActorRef out, GameState gameState, Tile tile){
        BasicCommands.addPlayer1Notification(out, "You summoned Fire Spitter on tile [ "+(tile.getTilex()+1) + ", " + (tile.getTiley()+1) + " ].", 1);
        ...
        gameState.addHumanUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_fire_spitter, gameState.getNewUnitID(), unitFireSpitter.class));
        ...
        try {Thread.sleep(1000);} catch (InterruptedException e) {e.printStackTrace();}
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setAttack(3);
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setHealth(2);
        BasicCommands.setUnitAttack(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 3);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        BasicCommands.setUnitHealth(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 2);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setName("Fire Spitter");
