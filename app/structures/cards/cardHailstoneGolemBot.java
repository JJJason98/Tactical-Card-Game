...
public class cardHailstoneGolemBot extends Card{
    public void UseCard(ActorRef out, GameState gameState, Tile tile) {
        BasicCommands.addPlayer1Notification(out, "Enemy summoned  Hailstone Golem on tile " + "[" + (tile.getTilex() + 1) + ", " + (tile.getTiley() + 1) + " ].", 3);
        try {
            Thread.sleep(1000);
        ...
        gameState.addBotUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_hailstone_golem, gameState.getNewUnitID(), unitHailstoneGolem.class));
        ...
        gameState.getBotUnits().get(gameState.getBotUnits().size()-1).setHealth(6);
        BasicCommands.setUnitAttack(out, gameState.getBotUnits().get(gameState.getBotUnits().size()-1), 4);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        BasicCommands.setUnitHealth(out, gameState.getBotUnits().get(gameState.getBotUnits().size()-1), 6);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        ...
        gameState.getBotUnits().get(gameState.getBotUnits().size()-1).setName("Hailstone Golem");
