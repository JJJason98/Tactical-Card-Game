...
import structures.units.unitComodoCharger;
import utils.BasicObjectBuilders;
import utils.StaticConfFiles;

public class cardComodoCharger extends Card{
	int manacost =1;
	
	public void UseCard(ActorRef out, GameState gameState, Tile tile) {
		BasicCommands.addPlayer1Notification(out, "You summoned Comodo Charger on tile [ "+(tile.getTilex()+1) + ", " + (tile.getTiley()+1) + " ].", 1);
	    ...
	    gameState.addHumanUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_comodo_charger, gameState.getNewUnitID(), unitComodoCharger.class));
	    ...
	    gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setName("Comodo Charger");
