...
public class cardIroncliffGuardian extends Card{
    ...
    public void UseCard(ActorRef out, GameState gameState, Tile tile){
        BasicCommands.addPlayer1Notification(out, "You summoned Ironcliff Guardian on tile [ "+(tile.getTilex()+1) + ", " + (tile.getTiley()+1) + " ].", 1);
        ...
        gameState.addHumanUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_ironcliff_guardian, gameState.getNewUnitID(), unitIroncliffGuardian.class));
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setName("IroncliffGuardian");
