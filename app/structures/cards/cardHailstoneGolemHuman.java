...
import structures.units.unitHailstoneGolem;
import utils.BasicObjectBuilders;
import utils.StaticConfFiles;

public class cardHailstoneGolemHuman extends Card {
    public void UseCard(ActorRef out, GameState gameState, Tile tile) {
        BasicCommands.addPlayer1Notification(out, "You summoned  Hailstone Golem on tile " + "[" + (tile.getTilex() + 1) + ", " + (tile.getTiley() + 1) + " ].", 3);
        try {
            Thread.sleep(1000);
        ...
        gameState.addHumanUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_hailstone_golem, gameState.getNewUnitID(), unitHailstoneGolem.class));
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setHealth(6);
        BasicCommands.setUnitAttack(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 4);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        BasicCommands.setUnitHealth(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 6);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setName("Hailstone Golem");
