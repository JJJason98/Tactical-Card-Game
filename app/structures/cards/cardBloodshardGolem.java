...
public class cardBloodshardGolem extends Card{
    int manacost=3;
    public void UseCard(ActorRef out, GameState gameState, Tile tile){
        BasicCommands.addPlayer1Notification(out, "Enemy summoned Bloodshard Golem on tile " + "["+(tile.getTilex()+1) + ", " + (tile.getTiley()+1) + " ].", 3);
        ...
        gameState.addBotUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_bloodshard_golem, gameState.getNewUnitID(), unitBloodshardGolem.class));
        ...
        gameState.getBotUnits().get(gameState.getBotUnits().size()-1).setName("Bloodshard Golem");
