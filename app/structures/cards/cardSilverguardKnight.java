...
public class cardSilverguardKnight extends Card{

    public void UseCard(ActorRef out, GameState gameState, Tile tile){
        BasicCommands.addPlayer1Notification(out, "You summoned Silverguard Knight on tile [ "+(tile.getTilex()+1) + ", " + (tile.getTiley()+1) + " ].", 1);
        ...
        gameState.addHumanUnits(BasicObjectBuilders.loadUnit(StaticConfFiles.u_silverguard_knight, gameState.getNewUnitID(), unitSilverguardKnight.class));
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setHealth(5);
        BasicCommands.setUnitAttack(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 1);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        BasicCommands.setUnitHealth(out, gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1), 5);
        try {Thread.sleep(10);} catch (InterruptedException e) {e.printStackTrace();}
        ...
        gameState.getHumanUnits().get(gameState.getHumanUnits().size()-1).setName("Silverguard Knight");
