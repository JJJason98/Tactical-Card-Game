...
public class unitFireSpitter extends Unit {
        ...
        if (attacked) return false;
            ...
            if (tile.getUnit() != null && !tile.getUnit().getOwner().equals("human")){
                if (tile.getUnit().getName().equals("Silverguard Knight") || tile.getUnit().getName().equals("Ironcliff Guardian") || tile.getUnit().getName().equals("Rock Pulveriser")){
                    ...
                    for (int i = 0; i < 9; i++) {
                        for (int j = 0; j < 5; j++) {
                            if (gameState.getBoard()[i][j].getUnit() != null && !gameState.getBoard()[i][j].getUnit().getOwner().equals("human")){
    ...
    public void attackUnit(ActorRef out, GameState gameState, Tile tile) {
        ...
        // if target not die, counter attack if the current unit is in target's attack range
            ...
            if (tile.getUnit().getName().equals("Pyromancer") || (Math.abs(this.getTile().getTilex() - tile.getTilex()) <= 1 && Math.abs(this.getTile().getTiley() - tile.getTiley()) <= 1)){
                ...
                if (tile.getUnit().getName().equals("Pyromancer")){
